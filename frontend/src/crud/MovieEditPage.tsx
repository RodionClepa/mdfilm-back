...
export function MovieEditPage() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const m = await endpoints.movies.get(id);
      setItem(m);
      setForm({
        title: m.title ?? '',
  ...
  async function onSave() {
    ...
    setLoading(true);
    try {
      await endpoints.movies.update(id, {
      ...
      {item && (
        <>
          ...
          <div className="row">
            ...
            <input type="date" value={form.releaseDate} onChange={(e) => setForm((f) => ({ ...f, releaseDate: e.target.value }))} disabled />
          </div>
          <div className="row">
